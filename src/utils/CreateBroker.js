...
function 港股代收费项目(总交易额) {
  ...
  const 交易印花税 = 总交易额 * 0.0013 > 1 ? 总交易额 * 0.0013 : 1;
...
export function countBrokerProfit(data) {
  // {
  //     name: '蒸发量',
  //     type: 'bar',
  ...
  let arr = [];
  data.forEach(l => {
    arr = arr.concat(l.list);
  });

  const res = new Map();

  let s = arr.reduce((list, item) => {
    if (res.has(item.券商)) {
      res.get(item.券商).申购成本 += item.申购成本;
      res.get(item.券商).盈亏 += item.盈亏;
      res.get(item.券商).中签数 += item.中签数
    } else {
      let o = { ...item }
      list.push(o)
      res.set(item.券商, o)
